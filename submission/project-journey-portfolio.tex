...
\noindent We also asked the professor who had answered our first interview
round with the most enthusiasm what they made of the finished tool. The reply
...
drafted mail before sending it, which is what the disclaimer from
Section~\ref{sec:users} asks them to do. The exchange is in
...
\noindent The design also landed with the side that ended the first version.
Roughly half of the professors we interviewed had rejected the central platform.
When we showed the finished skill to one of them, the answer was that it helps
professors too, ``if it's just by reducing the amount of untargeted mails we
get''. It is a single reply and not based on a run of the tool, but it is the
first sign that a product which asks nothing of professors can still be
