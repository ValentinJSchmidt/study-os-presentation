...
account.  We ran a session in the course and then a public beta, but we did not receive many replies.
However, the feedback we got ( also one from another professor) was positive and confirmed the two design decisions.
\end{tcolorbox}
...
Therefore, we have planned and developed a first full-stack prototype, with a backend, a database, and a web frontend. Since we are in the age of AI, we also planned to integrate a fully custom AI agent with custom tools that could supervise students during the search process and the exploration of their own possibilities and goals for their thesis. 
...
\noindent To mitigate this discrepancy, we planned a user study for professors, where they could give us their point of view without rating the actual idea itself. We wanted to understand the reasons behind the missing thesis list and whether it was intended or just fell off due to the lack of time.



\noindent While the whole application got stripped apart during the process of the course, the initial idea was a good starting point and a problem worth solving.
...
\begin{itemize}
 ...
 depend on. We are in the middle and wanted to satisfy both sides.
  \item We interviewed a substantial number of professors via E-Mail.  We sent 4 probe e-mails to professors we knew, then we automatically sent e-mails to all the professors in the department of computer science. This took place from late April till May.  We received a lot of feedback, which we documented and analyzed. The main insights are summarized below.
 The E-Mails can be found in the Appendix~\ref{app:prof-interview}. The professors were contacted via the official university e-mail addresses. We sent out two different variants of the e-mail, depending on whether the chair already had a public list of thesis topics or not. The professors were asked to give us their honest feedback on the current situation and the proposed idea.
 Main documented
 insights:
    \begin{itemize}
      \item Roughly half rejected a central platform outright and only about a
 third were conditionally open to it.
 The reason for rejection was that the professors think that fitting a student to a thesis topic is a personal process and that they want to get to know the student first.
 A comparable topic-listing platform at the university had already failed for incentive reasons rather than UI reasons, as can be seen by this quote: "the old system was the exact opposite: Listing theses was cumbersome. The list wasn’t maintained. And being among the few groups that listed on the central page usually just meant that the less engaged, less informed students would approach you first."
      \item One professor reacted negatively to our e-mail and thought we were too demanding. One important lesson is that when you send out a request to professors, you should be very careful with the wording and the tone of the message.
   
    \end{itemize}
 So we learned that the best way to please professors is to not ask them to do much at all. They can still go on and update their own website, but we don't necessarily need their input. 
...
\subsection*{Now what}
\begin{itemize}
  \item The main insight was that we could not work with a central list, but should focus on what is publicly available and on the students' side.
  \item This brought us to the idea of building an advisor that helps students, as detailed in the next section.
\end{itemize}
...
\subsection*{What}
At first, we tried to find a problem that fit our rough goal, a problem we had and knew from our own student life. Finding the right bachelor's or master's thesis can be difficult. 
At first we thought that the problem was that there was no central list of topics. 
...
\subsection*{So what}
\begin{itemize}
  \item We therefore discussed multiple pivots; however, we still wanted to build an app that would have been deployed. It would have been a full-stack app with a database and a backend.
  \item We discovered a new formulation of the problem: 
 \begin{infobox}[We solve $X$ for $Y$ by doing $Z$]
  We solve \textbf{the problem of finding a suitable thesis topic} for \textbf{students} by \textbf{providing a tool that helps students find and reflect on their thesis by providing information and ideas via an LLM}
\end{infobox}
...
\subsection*{Now what}
\begin{itemize}
  \item We now had a clear idea of the problem and started building an app. We wanted to assess the students based on different axes, like interest or skill. We also scraped papers by the professors and their chairs. This was all done so we might use local models or have a big database that we could use to answer the students' questions.
  \item You can find the app in the git logs of our repository. However, since we later decided not to use an app but a skill and then even later pivoted to a skill without any database ( even in text form) whatsoever, we have deleted it from the current iteration of the repository.
...
Since the initial idea was built on the supply of professors, we had to rethink our approach because the problem for the students still exists. The core problem is not the absence of a central thesis-topic list. Students often lack the context to decide which chair or company fits their interests, methods, skills, constraints, and thesis style. This makes thesis search highly individual: students need guided matching, not just another static list. Since this problem is highly individual and needs deep information about the user, we thought about a private assistant.
...
After further discussion in the team, we embraced the idea of a Skill, because a Skill can be inspected and adapted through plain Markdown, lowering the barrier for future maintainers.

\noindent Additionally, we don't have to think about security, privacy, deployment, and cloud billing since this is already handled by your LLM provider of choice. This realization then led to the initial 
...
both skills, the one with the Markdown database and the one without. The feedback from that session was spoken rather than written, and it pointed to the variant without the database as the more useful one. This led to the final decision of removing the Markdown database and using the slimmer version with the minimal maintenance overload. The session itself is described in Section~6.
...
\noindent We also anticipated professors complaining about AI spam, so we instructed the skill to output a clear disclaimer in order to encourage students to at least modify the generated e-mails and drafts.
...
account, since it runs inside an LLM assistant people already use. The same
conversations favoured the database-free variant. Only the course professor answered in writing.
...
\noindent We also asked a professor who had answered our first interview
round what they made of the finished tool. The reply
came before they had run it, so it judges the approach from our description and
the repository rather than a session. It called the skill a great idea that
helps professors as well as students, and expected most students to edit the
drafted mail, which is what the disclaimer from
Section~\ref{sec:users} asks them to do. 
This is the first sign that a product which asks nothing of professors can still be
something they want to exist, and professors who see a benefit for themselves
are the people who can point students to it. The exchange is in
Appendix~\ref{app:prof-reply}.

\subsection*{So what}

\noindent The feedback confirmed the two decisions the project rests on. The
live search finds things a real user did not know,  and the property people valued most
...
have fixed the session ending too early, which was the actual problem. Still, professors can list thesis topics on their own website, and the skill will find them.
...
first was the pivot described in Sections~2 and~3, when the professor's interviews
...
\repofile{AGENTS.md}, so it can be improved with little human intervention.

\noindent The last thing we did was test whether any of this is specific to
these. The skill is a sequence that does not depend on the subject. It
...
\noindent The next steps are mostly making the tool more known and taking feedback from users. We have started to reach out and are still waiting for answers as of today. 
We hope that the skill will see use by students. Since it is easily maintainable, we hope that it will be used for a long time and that it will be improved by future students.
% --------------------------------------------------------------------------- Appendix
\appendix
...
conducted by e-mail. We sent two variants depending on whether the chair had already
...
reflection actually happens, and teaching staff is not flooded with AI slop.

\medskip
\noindent Would it be possible to mention these skills at the information events
on these? Or would we have to contact somebody else for that?
...
% --------------------------------------------------------------------------- Appendix E
\section{Reply from a Professor on the Finished Tool}
\label{app:prof-reply}

\noindent Sent in English on 10 September 2026 to one of the professors from the
interview round in Appendix~\ref{app:prof-interview}. Both mails are reproduced
verbatim. The professor's name, the e-mail addresses, and the signature are
removed for data protection reasons.

\subsection*{Our mail}

\noindent Hello Prof [anonymised],

\medskip
\noindent We are finalizing our project now, and if you want to, you can give us
your honest opinion on its outcome.

\medskip
\noindent Taking your feedback to heart, we shifted away from the central
platform and have now developed a skill for Chatbots like Claude, ChatGPT, etc.,
that might help students find their thesis.

\medskip
\noindent You can find it here: \url{https://github.com/Tue-StudyOS/study-os-thesis}\\
It works quite well, and most importantly, you do not need to do anything
different. It builds a profile of the student and then recommends a thesis and
drafts an email to the potential supervisor.

\medskip
\noindent Its outputs are also labeled as done by AI, so we also had guarding
against lots of ``slop'' emails in mind.

\medskip
\noindent The whole process takes around 30 minutes, so students at least do
some reflection on what they want to do.

\medskip
\noindent Do you think we (at least partly) have addressed your concerns
regarding a tool that helps students find their thesis?

\medskip
\noindent Your honesty last time was of great help to us, and we are open to any
criticism or insight. It really helped us understand the professor's side.

\medskip
\noindent We hope we will be able to make the skill more well-known within the
department, since we genuinely think it can help with finding a thesis and it
works better than just using AI from the start.

\medskip
\noindent We thank you so much for your time ( and don't worry, the semester
project is also almost over now).

\medskip
\noindent Best Regards, Valentin Schmidt

\subsection*{The reply}

\noindent Dear Mr. Schmidt,

\medskip
\noindent Thanks for your kind mail! I couldn't yet try your tool, but from what
you write below, and on GitHub, it sounds like a great idea that doesn't just help
students, but also the professors, and if it's just by reducing the amount of
untargeted emails we get.

\medskip
\noindent I hope that you'll get a lot of users! And I'll watch out for emails
from the tool (although I guess that most students will want to at least edit
the email to sound more genuine. But that's fine)
