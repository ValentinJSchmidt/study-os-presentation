...
\begin{frame}{The plan we started with --- and why we abandoned it}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      So instead of maintaining data, we packaged \emph{the search process}:
    a portable agent skill that uses the live web as its source of truth.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{The idea: ship the method, not the data}
  ...
  (Claude, ChatGPT, Gemini). No database, no account, no custom app. The student
  just asks for thesis help, and the agent \alert{walks them through three steps}:
  \begin{enumerate}
...
\begin{frame}{How it works: one entry point, two tracks}
  ...
  \begin{itemize}
    ...
    \item Profile dimensions: \emph{interests $\cdot$ methods $\cdot$ domain
      $\cdot$ thesis style $\cdot$ skills $\cdot$ no-go's}
...
\begin{frame}{Where we are, and what comes next}
  ...
  \begin{itemize}
    ...
    \item Dropped the full-stack app; built the skill-based tool instead
  ...
  \begin{itemize}
    ...
    \item Getting it into students' hands --- starting with the CS Fachschaft and Bachelor's/Master's thesis information session.
  \end{itemize}
\end{frame}
